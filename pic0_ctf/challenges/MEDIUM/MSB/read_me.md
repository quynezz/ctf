...
```bash 
python sigBits.py -i Ninja-and-Prince-Genji-Ukiyoe-Utagawa-Kunisada.flag.png -t MSB -out flag
```

Print out the printable text format of the extracted file 
```bash 
strings flag | grep -oE "picoCTF{.*?}"
```
